...
public abstract class AbstractPaymentTests implements RestClientTest {
	...
	private BasePayment createTestBasePayment() {
		...
		Payer payer = new PayerBuilder().withAllowedPaymentInstruments(List.of(PaymentInstrument.BANK_ACCOUNT))
		...
		return builder.withCallback(url + "notify", url + "return")
				.order("123", 10L, Currency.CZK, "description")
				.inLang(Lang.EN)
	...
	private BasePayment createBnplTestBasePayment() {
		...
		Payer payer = new PayerBuilder().withAllowedPaymentInstruments(
						Arrays.asList(PaymentInstrument.TWISTO, PaymentInstrument.SKIPPAY))
	...
	protected void testGenerateStatement(IGPConnector connector) {
		...
		try {
			byte[] statement = connector.getAppToken(TestUtils.CLIENT_ID, TestUtils.CLIENT_SECRET)
